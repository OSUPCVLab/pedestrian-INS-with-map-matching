function [xReconstructed, time] = construct_missed_samples(xMag, G_Dt, packetCount)
...
xReconstructed = zeros(n,1); xReconstructed(1) = xMag(1);
...
for i=2:n1
    ...
    if (diff == 2)
        ...
        xReconstructed(i+j) = xMag(i);
        xReconstructed(i+j-1) = (xMag(i)+xMag(i-1))/2.0; % interpolation
