...
if (choice == 1)
    ...
    pedestrianTraverseName = sprintf(fileName, choice2);
        ...
        fileName = '../caldwell_lab_wired_57600_multiple.txt';
        pedestrianTraverseName = sprintf(fileName, choice2);
    else
        fprintf('Invalid input!\n');
        pause();
...
nMissedSamples = number_of_missed_samples(packetCount);
if ( nMissedSamples > 0 )
% recover missed samples (XBee or wired connection) by using linear interpolation
...
%% FISCHER 9-state EKF with ZUPT aid start: Initialise parameters.
...
% TAHA modification for sample to stride convertion: Mehmet, you can plot
% 0s and 1s regarding stance and swing phases, and then run over the data
% with a median or running average filter to eliminate the outliers, and
% finally take the middle stance sample as the stride. If you implement
% your own strider detector, remove mine.
...
for t = 2:data_size
    ...
    if ( norm(gyro_s(:,t)) < gyro_threshold )
        % TAHA stride detection: Mehmet remove this script if you implement
        % your own stride detection method.
        z = z+1;
        ZUPTindexes(z) = t;
        if (z > 1)
...
if (choice == 1)
    if (choice2 == 8)
        sf = 1.07; % scale factor for distance travelled
        angle = 190;
    end
else if (choice == 2)
    if (choice2 == 1)
...
% scale position data because pedetrian speed varies in each experiment
pos_r = sf*pos_r;
if (choice == 1)
    pos_r(1,:) = -pos_r(1,:);
else if (choice == 2)
...
imshow(img);
hold on;
% plot raw trajectory on the indoor map - all samples
...
%%
figure(4); set(figure(4),'position',[481 14 1114 799]);
set(gcf,'position',[564 61 927 670]);
imshow(img); hold on;
...
hold off;
legend('Error state EKF with ZUPT aid','Non-recursive Bayesian map-matching','start','stop');
set(legend,'fontsize',16,'interpreter','latex','location','northeast');

%%
figure(5); set(gcf,'position',[371 151 535 422]);
imshow(staticFloorPDF{choice},'colormap',jet(255));
colormap jet; set(gca, 'YDir', 'reverse'); hold on;
