...
choice = input('Enter 1 for Bolz Hall, 2 for Caldwell, 3 for OVAL: ');
...
%       gyroDataForBiasEstimationName = '../around pcv lab multiple times/around_pcv_gyro_data_for_bias_estimation_wired_57600_multiple.txt';
        fileName = '../oval_gyro_calibration_%i.txt';
        gyroDataForBiasEstimationName = sprintf(fileName, choice2);
    else 
...
if (choice == 1)
    ...
    else if (choice2 == 6) % room traversal
        sf = 1.06; angle = 184;
    else if (choice2 == 7)
        sf = 1.06; angle = 187;
        else if (choice2 == 8)
...
%     end
        ...
        img = imread(buildingMaps{choice});
    end
end
...
if (choice == 1)
    if (choice2 == 50)
        ...
        if (choice2 == 1)
            startPixelCoordinates{3} = [300 + 1; 200 + 1];
        else if (choice2 == 2)
                startPixelCoordinates{3} = [560 + 1; 160 + 1];
            else if (choice2 == 3)
                    startPixelCoordinates{3} = [300 + 1; 200 + 1];
                end
            end
...
meter2pixelConstant{3} = 306.77 / 140.8;
...
h = fspecial('average',[1 15]);
trajectoryStridesPixelsB = trajectoryStridesPixels;
trajectoryStridesPixelsB(1,:) = imfilter(trajectoryStridesPixels(1,:),h,'replicate');
trajectoryStridesPixelsB(2,:) = imfilter(trajectoryStridesPixels(2,:),h,'replicate');
...
% plot(trajectoryPixels(1,ZUPTindexes),trajectoryPixels(2,ZUPTindexes), 'gx','linewidth',2);
% strides with blue circles
plot(trajectoryStridesPixels(1,:),trajectoryStridesPixels(2,:), 'r.-','linewidth',1.5);
plot(trajectoryStridesPixelsB(1,:),trajectoryStridesPixelsB(2,:), 'g-','linewidth',1.5);
hold off;
...
% 
% %%
% load('fischer_ESKF_bolz_hall_dissertation.mat');
% figure(7); clf; set(gcf,'position',[564 61 927 670]);
% imshow(img); hold on;
% % plot(traj_1(1,1:end),traj_1(2,1:end),'g-.','linewidth',2);
% plot(traj_3(1,1:end),traj_3(2,1:end),'b-o','linewidth',1.5,'markersize',5);
% plot(traj_2(1,1:end),traj_2(2,1:end),'r-x','linewidth',1.5);
% plot(276, 522, 'ks','linewidth',2,'markerfacecolor','g','markersize',10);
% hold off;
% legend('error state KF with ZUPT  - experiment 1',...
%     'error state KF with ZUPT  - experiment 2', 'actual stop');
% set(legend,'fontsize',20,'interpreter','latex','location','northeast');
% 
% figure(8); clf; set(gcf,'position',[564 61 927 670]);
% trim = 135;
% imshow(img(trim:end,1:end)); hold on;
% % plot(traj_56(1,1:end),traj_56(2,1:end),'g-x','linewidth',2);
% plot(traj_53(1,1:end),traj_53(2,1:end)-trim,'b-o','linewidth',1.5,'markersize',5);
% plot(traj_54(1,1:end),traj_54(2,1:end)-trim,'r-x','linewidth',1.5);
% % plot(276, 522, 'ks','linewidth',2,'markerfacecolor','g','markersize',10);
% hold off;
% legend('error state KF with ZUPT  - experiment 3',...
%     'error state KF with ZUPT  - experiment 4');
% set(legend,'fontsize',20,'interpreter','latex','location','northeast');
% 
% % clear traj_1 traj_2 traj_3 traj_53 traj_54 traj_56
% load('fischer_ESKF_caldwell_dissertation.mat');
% figure(9); clf; set(gcf,'position',[564 61 927 670]);
% imshow(img); hold on;
% plot(traj_0(1,1:end),traj_0(2,1:end),'g-d','linewidth',1.5,'markersize',5);
% plot(traj_1(1,1:end),traj_1(2,1:end),'b-o','linewidth',1.5,'markersize',5);
% % plot(traj_4(1,1:end),traj_4(2,1:end),'r-x','linewidth',1.5);
% plot(traj_9(1,1:end),traj_9(2,1:end),'r-x','linewidth',1.5,'markersize',5);
% hold off;
% legend('error state KF with ZUPT  - experiment 1',...
%     'error state KF with ZUPT  - experiment 2',...
%     'error state KF with ZUPT  - experiment 3');
