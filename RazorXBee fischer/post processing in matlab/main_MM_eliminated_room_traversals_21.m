...
strideIndexesTemp = zeros(1,length(strideIndexes)+1); % recovering 1 missed stride
% the stride between 55 and 56 is missed so we derive 56 and hence 56 becomes 57
strideIndexesTemp(1:155) = strideIndexes(1:155);
strideIndexesTemp(156) = (strideIndexes(155) + strideIndexes(156)) / 2;
strideIndexesTemp(157:end) = strideIndexes(156:end);
strideIndexes = strideIndexesTemp;
clear strideIndexesTemp;
...
if (choice == 1)
    ...
    sf = 1.04; % scale factor
    angle = 180; % 181   1892     190 for multiple traverse
    else if (choice == 3)
...
load('bolz_hall_pdf_updated2.mat');
...
str = 157;
plot(trajectoryPixels(1,strideIndexes(str)),trajectoryPixels(2,strideIndexes(str)),'go-','linewidth',2);
str2 = 121;
plot(trajectoryPixels(1,strideIndexes(str2)),trajectoryPixels(2,strideIndexes(str2)),'ro-','linewidth',2);
str3 = 211;
plot(trajectoryPixels(1,strideIndexes(str3)),trajectoryPixels(2,strideIndexes(str3)),'mo-','linewidth',2);
str4 = 164;
plot(trajectoryPixels(1,strideIndexes(str4)),trajectoryPixels(2,strideIndexes(str4)),'bo-','linewidth',2);
str5 = 235;
plot(trajectoryPixels(1,strideIndexes(str5)),trajectoryPixels(2,strideIndexes(str5)),'yo-','linewidth',2);
str6 = 295;
plot(trajectoryPixels(1,strideIndexes(str6)),trajectoryPixels(2,strideIndexes(str6)),'ko-','linewidth',2);
hold off;
...
plot(finalEstimate(1,1:65),finalEstimate(2,1:65),'r-x','linewidth',1.5);
...
plot(finalEstimate(1,strideNumber-278),finalEstimate(2,strideNumber-278),'kp','linewidth',2,'markerfacecolor','g','markersize',16);
plot(finalEstimate(1,strideNumber-277),finalEstimate(2,strideNumber-277),'kv','linewidth',2,'markerfacecolor','y','markersize',16);
plot(finalEstimate(1,strideNumber-276),finalEstimate(2,strideNumber-276),'ko','linewidth',2,'markerfacecolor',[1 0.5 0],'markersize',16);
plot(finalEstimate(1,strideNumber-275),finalEstimate(2,strideNumber-275),'ks','linewidth',2,'markerfacecolor',[1 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-274),finalEstimate(2,strideNumber-274),'kh','linewidth',2,'markerfacecolor',[0 0.5 1],'markersize',16);
plot(finalEstimate(1,strideNumber-273),finalEstimate(2,strideNumber-273),'kd','linewidth',2,'markerfacecolor',[0.5 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-272),finalEstimate(2,strideNumber-272),'k^','linewidth',2,'markerfacecolor',[0 0.5 0],'markersize',16);
...
hkmin3angle = 22; % rotation for hypothesis k-3, computed wrt k-3 and k-4
hkmin2angle = -3; % rotation for hypothesis k-2.
hkmin1angle = -17; % rotation for hypothesis k-1.
hkangle = -15; % rotation for hypothesis k.
hkplus1angle = 30; % rotation for hypothesis k+1.
% burda kaldik kplus2
hkplus2angle = 60; % rotation for hypothesis k+2.
hkplus3angle = 73; % rotation for hypothesis k+3.
...
hkmin3Translation = [-8;40]; % [-137;273]
finalEstimateRotatedTranslatedhkmin3 = finalEstimateRotatedhkmin3 + hkmin3Translation; % translation for hypothesis k-3
hkmin2Translation = finalEstimateRotatedhkmin2(:,strideNumber-277)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
finalEstimateRotatedTranslatedhkmin2 = finalEstimateRotatedhkmin2 - hkmin2Translation; % translation for hypothesis k-2
hkmin1Translation = finalEstimateRotatedhkmin1(:,strideNumber-276)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
finalEstimateRotatedTranslatedhkmin1 = finalEstimateRotatedhkmin1 - hkmin1Translation; % translation for hypothesis k-2
hkTranslation = finalEstimateRotatedhk(:,strideNumber-275)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
finalEstimateRotatedTranslatedhk = finalEstimateRotatedhk - hkTranslation; % translation for hypothesis k-2
hkplus1Translation = finalEstimateRotatedhkplus1(:,strideNumber-274)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
finalEstimateRotatedTranslatedhkplus1 = finalEstimateRotatedhkplus1 - hkplus1Translation; % translation for hypothesis k-2
hkplus2Translation = finalEstimateRotatedhkplus2(:,strideNumber-273)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
finalEstimateRotatedTranslatedhkplus2 = finalEstimateRotatedhkplus2 - hkplus2Translation; % translation for hypothesis k-2
hkplus3Translation = finalEstimateRotatedhkplus3(:,strideNumber-272)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278);%[-30; 140];
...
if (choice == 1 || choice == 2)
    if (showRotated)
        plot(finalEstimateRotatedhk(1,1:end),finalEstimateRotatedhk(2,1:end),'r-x','linewidth',1.5);
        plot(finalEstimateRotatedhk(1,strideNumber-274),finalEstimateRotatedhk(2,strideNumber-274),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhk(1,strideNumber-275),finalEstimateRotatedhk(2,strideNumber-275),'ks','linewidth',2,'markerfacecolor','y','markersize',16);
        % we can take stride k-2 to calculate the heading of the current hypothesis
        plot(finalEstimateRotatedhk(1,strideNumber-276),finalEstimateRotatedhk(2,strideNumber-276),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
    end
else (choice == 3)
%plot(finalEstimateRotated(1,1:end),finalEstimateRotated(2,1:end),'r-x','linewidth',1.5);
end
plot(finalEstimateRotatedTranslatedhkmin3(1,strideNumber-278:strideNumber-278+probStrides),...
    finalEstimateRotatedTranslatedhkmin3(2,strideNumber-278:strideNumber-278+probStrides),'g-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin2(1,strideNumber-277:strideNumber-277+probStrides),...
    finalEstimateRotatedTranslatedhkmin2(2,strideNumber-277:strideNumber-277+probStrides),'y-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin1(1,strideNumber-276:strideNumber-276+probStrides),...
    finalEstimateRotatedTranslatedhkmin1(2,strideNumber-276:strideNumber-276+probStrides),'y-x','linewidth',1.5,'color',[1 0.5 0]);
plot(finalEstimateRotatedTranslatedhk(1,strideNumber-275:strideNumber-275+probStrides),...
    finalEstimateRotatedTranslatedhk(2,strideNumber-275:strideNumber-275+probStrides),'y-x','linewidth',1.5,'color',[1 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus1(1,strideNumber-274:strideNumber-274+probStrides),...
    finalEstimateRotatedTranslatedhkplus1(2,strideNumber-274:strideNumber-274+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 1]);
plot(finalEstimateRotatedTranslatedhkplus2(1,strideNumber-273:strideNumber-273+probStrides),...
    finalEstimateRotatedTranslatedhkplus2(2,strideNumber-273:strideNumber-273+probStrides),'y-x','linewidth',1.5,'color',[0.5 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus3(1,strideNumber-272:strideNumber-272+probStrides),...
    finalEstimateRotatedTranslatedhkplus3(2,strideNumber-272:strideNumber-272+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 0]);
...
for i=1:probStrides
    probStridesVectorhkmin3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin3(:,strideNumber-278+i),0,0,0,0);
    probStridesVectorhkmin2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin2(:,strideNumber-277+i),0,0,0,0);
    probStridesVectorhkmin1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin1(:,strideNumber-276+i),0,0,0,0);
    probStridesVectorhk(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhk(:,strideNumber-275+i),0,0,0,0);
    probStridesVectorhkplus1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus1(:,strideNumber-274+i),0,0,0,0);
    probStridesVectorhkplus2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus2(:,strideNumber-273+i),0,0,0,0);
    probStridesVectorhkplus3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus3(:,strideNumber-272+i),0,0,0,0);
end
...
firstStride = 50; % 18
lastStride = strideNumber-275; % 16
...
plot(mu(1,1:end),mu(2,1:end),'b-x','linewidth',1.5); % this is IMU (EKF+DR+ZUPT) solution
%plot(muUS(1,1:end-18*upsamplingRate),muUS(2,1:end-18*upsamplingRate),'b-.','linewidth',3); % this is IMU (EKF+DR+ZUPT) solution
plot(finalEstimate(1,1:140),finalEstimate(2,1:140),'r-x','linewidth',1.5);
plot(finalEstimate(1,1),finalEstimate(2,1),'ro','linewidth',2,'markerfacecolor','r','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,end),finalEstimate(2,end),'gs','linewidth',2,'markerfacecolor','g','markeredgecolor','k','markersize',16);
...
% Room 216 traversal is over. The pedestrian is walking to Room 224 now.
%% illustration of hypotheses
...
plot(finalEstimate(1,1),finalEstimate(2,1),'ro','linewidth',2,'markerfacecolor','r','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,end),finalEstimate(2,end),'gs','linewidth',2,'markerfacecolor','g','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,strideNumber-232),finalEstimate(2,strideNumber-232),'kp','linewidth',2,'markerfacecolor','g','markersize',16);
plot(finalEstimate(1,strideNumber-231),finalEstimate(2,strideNumber-231),'kv','linewidth',2,'markerfacecolor','y','markersize',16);
plot(finalEstimate(1,strideNumber-230),finalEstimate(2,strideNumber-230),'ko','linewidth',2,'markerfacecolor',[1 0.5 0],'markersize',16);
plot(finalEstimate(1,strideNumber-229),finalEstimate(2,strideNumber-229),'ks','linewidth',2,'markerfacecolor',[1 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-228),finalEstimate(2,strideNumber-228),'kh','linewidth',2,'markerfacecolor',[0 0.5 1],'markersize',16);
plot(finalEstimate(1,strideNumber-227),finalEstimate(2,strideNumber-227),'kd','linewidth',2,'markerfacecolor',[0.5 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-226),finalEstimate(2,strideNumber-226),'k^','linewidth',2,'markerfacecolor',[0 0.5 0],'markersize',16);
% plot(finalEstimate(1,18),finalEstimate(2,18),'k<','linewidth',2,'markerfacecolor','k','markersize',16);
else (choice == 3)
plot(mu(1,1:end-30),mu(2,1:end-30), 'b-.','linewidth',2); % this is IMU (EKF+DR+ZUPT) solution
plot(finalEstimate(1,:),finalEstimate(2,:), 'r-','linewidth',2);
...
%% hypothesis k-3, k-2, k-1, k, k+1, k+2, k+3
% the angles are calculated according to IMU location & actual door location
hkmin3angle = -90; % rotation for hypothesis k-3, computed wrt k-3 and k-4
hkmin2angle = -87; % rotation for hypothesis k-2.
hkmin1angle = -73; % rotation for hypothesis k-1.
hkangle = 0; % rotation for hypothesis k.
hkplus1angle = 2; % rotation for hypothesis k+1.
% burda kaldik kplus2
hkplus2angle = 55; % rotation for hypothesis k+2.
hkplus3angle = 100; % rotation for hypothesis k+3.
% for each hypothesis, the rotation matrix is wrt to actual door location
rotation_matrixkmin3angle = [cosd(hkmin3angle) -sind(hkmin3angle);
    sind(hkmin3angle) cosd(hkmin3angle)];
rotation_matrixkmin2angle = [cosd(hkmin2angle) -sind(hkmin2angle);
...
%door216Location = [760.4018;999.5193];
hkmin3Translation = [-705;-560]; % [-137;273]
finalEstimateRotatedTranslatedhkmin3 = finalEstimateRotatedhkmin3 + hkmin3Translation; % translation for hypothesis k-3
hkmin2Translation = finalEstimateRotatedhkmin2(:,strideNumber-231)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhkmin2 = finalEstimateRotatedhkmin2 - hkmin2Translation; % translation for hypothesis k-2
hkmin1Translation = finalEstimateRotatedhkmin1(:,strideNumber-230)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhkmin1 = finalEstimateRotatedhkmin1 - hkmin1Translation; % translation for hypothesis k-2
hkTranslation = finalEstimateRotatedhk(:,strideNumber-229)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhk = finalEstimateRotatedhk - hkTranslation; % translation for hypothesis k-2
hkplus1Translation = finalEstimateRotatedhkplus1(:,strideNumber-228)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhkplus1 = finalEstimateRotatedhkplus1 - hkplus1Translation; % translation for hypothesis k-2
hkplus2Translation = finalEstimateRotatedhkplus2(:,strideNumber-227)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhkplus2 = finalEstimateRotatedhkplus2 - hkplus2Translation; % translation for hypothesis k-2
hkplus3Translation = finalEstimateRotatedhkplus3(:,strideNumber-226)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232);%[-30; 140];
finalEstimateRotatedTranslatedhkplus3 = finalEstimateRotatedhkplus3 - hkplus3Translation; % translation for hypothesis k-2
probStrides = 10;
...
if (choice == 1 || choice == 2)
    if (showRotated)
        plot(finalEstimateRotatedhkplus1(1,1:end),finalEstimateRotatedhkplus1(2,1:end),'r-x','linewidth',1.5);
        plot(finalEstimateRotatedhkplus1(1,strideNumber-227),finalEstimateRotatedhkplus1(2,strideNumber-227),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhkplus1(1,strideNumber-228),finalEstimateRotatedhkplus1(2,strideNumber-228),'ks','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhkplus1(1,strideNumber-229),finalEstimateRotatedhkplus1(2,strideNumber-229),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        % we can take stride k-2 to calculate the heading of the current hypothesis
...
plot(finalEstimateRotatedTranslatedhkmin3(1,strideNumber-232:strideNumber-232+probStrides),...
    finalEstimateRotatedTranslatedhkmin3(2,strideNumber-232:strideNumber-232+probStrides),'g-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin2(1,strideNumber-231:strideNumber-231+probStrides),...
    finalEstimateRotatedTranslatedhkmin2(2,strideNumber-231:strideNumber-231+probStrides),'y-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin1(1,strideNumber-230:strideNumber-230+probStrides),...
    finalEstimateRotatedTranslatedhkmin1(2,strideNumber-230:strideNumber-230+probStrides),'y-x','linewidth',1.5,'color',[1 0.5 0]);
plot(finalEstimateRotatedTranslatedhk(1,strideNumber-229:strideNumber-229+probStrides),...
    finalEstimateRotatedTranslatedhk(2,strideNumber-229:strideNumber-229+probStrides),'y-x','linewidth',1.5,'color',[1 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus1(1,strideNumber-228:strideNumber-228+probStrides),...
    finalEstimateRotatedTranslatedhkplus1(2,strideNumber-228:strideNumber-228+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 1]);
plot(finalEstimateRotatedTranslatedhkplus2(1,strideNumber-227:strideNumber-227+probStrides),...
    finalEstimateRotatedTranslatedhkplus2(2,strideNumber-227:strideNumber-227+probStrides),'y-x','linewidth',1.5,'color',[0.5 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus3(1,strideNumber-226:strideNumber-226+probStrides),...
    finalEstimateRotatedTranslatedhkplus3(2,strideNumber-226:strideNumber-226+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 0]);
hold off; legend('hypothesis k-3 trajectory','hypothesis k-2 trajectory',...
    'hypothesis k-1 trajectory','hypothesis k trajectory',...
    'hypothesis k+1 trajectory','hypothesis k+2 trajectory',...
    'hypothesis k+3 trajectory');
...
probStridesVectorhkplus3 = zeros(1,probStrides);
for i=1:probStrides
    probStridesVectorhkmin3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin3(:,strideNumber-232+i),0,0,0,0);
    probStridesVectorhkmin2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin2(:,strideNumber-231+i),0,0,0,0);
    probStridesVectorhkmin1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin1(:,strideNumber-230+i),0,0,0,0);
    probStridesVectorhk(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhk(:,strideNumber-229+i),0,0,0,0);
    probStridesVectorhkplus1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus1(:,strideNumber-228+i),0,0,0,0);
    probStridesVectorhkplus2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus2(:,strideNumber-227+i),0,0,0,0);
    probStridesVectorhkplus3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus3(:,strideNumber-226+i),0,0,0,0);
end
...
yticklabels({'','.2','.4','.6','.8','1'});
set(gca,'GridLineStyle','--','position',[0.1442 0.1221 0.2345 0.3216]);
%% gradual correction for the trajectory in the room 224
% the hypothesis (stride) with the highest probability has the max rotation and 
% translation. the angle and the shift is gradually distributed to the strides in 
% the room. the very first stride in the room is applied least rotation and shift.
...
% hkplus2angle = -36; % this is the most voted hypothesis
firstStride = 123-1; % 18
lastStride = strideNumber - 229; % hypothesis with the highest probability
roomStrides = lastStride - firstStride;
translationReference = finalEstimateRotatedTranslatedhk(:,lastStride);
for i = 0:roomStrides
    angleTemp = hkangle*(roomStrides-i)/roomStrides;
...
set(legend,'fontsize',16,'interpreter','latex','location','northwest');
% figure(11);xlim([660 1410]);ylim([548 1018]);
% Room 224 traversal is over. The pedestrian is walking to PCV Lab now.
%% illustration of hypotheses
...
plot(finalEstimate(1,1),finalEstimate(2,1),'ro','linewidth',2,'markerfacecolor','r','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,end),finalEstimate(2,end),'gs','linewidth',2,'markerfacecolor','g','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,strideNumber-178),finalEstimate(2,strideNumber-178),'kp','linewidth',2,'markerfacecolor','g','markersize',16);
plot(finalEstimate(1,strideNumber-177),finalEstimate(2,strideNumber-177),'kv','linewidth',2,'markerfacecolor','y','markersize',16);
plot(finalEstimate(1,strideNumber-176),finalEstimate(2,strideNumber-176),'ko','linewidth',2,'markerfacecolor',[1 0.5 0],'markersize',16);
plot(finalEstimate(1,strideNumber-175),finalEstimate(2,strideNumber-175),'ks','linewidth',2,'markerfacecolor',[1 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-174),finalEstimate(2,strideNumber-174),'kh','linewidth',2,'markerfacecolor',[0 0.5 1],'markersize',16);
plot(finalEstimate(1,strideNumber-173),finalEstimate(2,strideNumber-173),'kd','linewidth',2,'markerfacecolor',[0.5 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-172),finalEstimate(2,strideNumber-172),'k^','linewidth',2,'markerfacecolor',[0 0.5 0],'markersize',16);
% plot(finalEstimate(1,18),finalEstimate(2,18),'k<','linewidth',2,'markerfacecolor','k','markersize',16);
else (choice == 3)
plot(mu(1,1:end-30),mu(2,1:end-30), 'b-.','linewidth',2); % this is IMU (EKF+DR+ZUPT) solution
plot(finalEstimate(1,:),finalEstimate(2,:), 'r-','linewidth',2);
...
%% hypothesis k-3, k-2, k-1, k, k+1, k+2, k+3
% the angles are calculated according to IMU location & actual door location
hkmin3angle = -50; % rotation for hypothesis k-3, computed wrt k-3 and k-4
hkmin2angle = -10; % rotation for hypothesis k-2.
hkmin1angle = 24; % rotation for hypothesis k-1.
hkangle = 25; % -8 or 25 rotation for hypothesis k.
hkplus1angle = -47; % rotation for hypothesis k+1.
% burda kaldik kplus2
hkplus2angle = -52; % rotation for hypothesis k+2.
hkplus3angle = -55; % rotation for hypothesis k+3.
% for each hypothesis, the rotation matrix is wrt to actual door location
rotation_matrixkmin3angle = [cosd(hkmin3angle) -sind(hkmin3angle);
    sind(hkmin3angle) cosd(hkmin3angle)];
rotation_matrixkmin2angle = [cosd(hkmin2angle) -sind(hkmin2angle);
...
%door216Location = [760.4018;999.5193];
hkmin3Translation = [-181;-518]; % [-137;273]
finalEstimateRotatedTranslatedhkmin3 = finalEstimateRotatedhkmin3 + hkmin3Translation; % translation for hypothesis k-3
hkmin2Translation = finalEstimateRotatedhkmin2(:,strideNumber-177)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhkmin2 = finalEstimateRotatedhkmin2 - hkmin2Translation; % translation for hypothesis k-2
hkmin1Translation = finalEstimateRotatedhkmin1(:,strideNumber-176)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhkmin1 = finalEstimateRotatedhkmin1 - hkmin1Translation; % translation for hypothesis k-2
hkTranslation = finalEstimateRotatedhk(:,strideNumber-175)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhk = finalEstimateRotatedhk - hkTranslation; % translation for hypothesis k-2
hkplus1Translation = finalEstimateRotatedhkplus1(:,strideNumber-174)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhkplus1 = finalEstimateRotatedhkplus1 - hkplus1Translation; % translation for hypothesis k-2
hkplus2Translation = finalEstimateRotatedhkplus2(:,strideNumber-173)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhkplus2 = finalEstimateRotatedhkplus2 - hkplus2Translation; % translation for hypothesis k-2
hkplus3Translation = finalEstimateRotatedhkplus3(:,strideNumber-172)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178);%[-30; 140];
finalEstimateRotatedTranslatedhkplus3 = finalEstimateRotatedhkplus3 - hkplus3Translation; % translation for hypothesis k-2
probStrides = 10;
...
if (choice == 1 || choice == 2)
    if (showRotated)
        plot(finalEstimateRotatedhk(1,1:end),finalEstimateRotatedhk(2,1:end),'r-x','linewidth',1.5);
        plot(finalEstimateRotatedhk(1,strideNumber-174),finalEstimateRotatedhk(2,strideNumber-174),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhk(1,strideNumber-175),finalEstimateRotatedhk(2,strideNumber-175),'ks','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhk(1,strideNumber-176),finalEstimateRotatedhk(2,strideNumber-176),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        % we can take stride k-2 to calculate the heading of the current hypothesis
        %plot(finalEstimateRotatedhkplus3(1,strideNumber-16),finalEstimateRotatedhkplus3(2,strideNumber-16),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
    end
else (choice == 3)
...
plot(finalEstimateRotatedTranslatedhkmin3(1,strideNumber-178:strideNumber-178+probStrides),...
    finalEstimateRotatedTranslatedhkmin3(2,strideNumber-178:strideNumber-178+probStrides),'g-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin2(1,strideNumber-177:strideNumber-177+probStrides),...
    finalEstimateRotatedTranslatedhkmin2(2,strideNumber-177:strideNumber-177+probStrides),'y-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin1(1,strideNumber-176:strideNumber-176+probStrides),...
    finalEstimateRotatedTranslatedhkmin1(2,strideNumber-176:strideNumber-176+probStrides),'y-x','linewidth',1.5,'color',[1 0.5 0]);
plot(finalEstimateRotatedTranslatedhk(1,strideNumber-175:strideNumber-175+probStrides),...
    finalEstimateRotatedTranslatedhk(2,strideNumber-175:strideNumber-175+probStrides),'y-x','linewidth',1.5,'color',[1 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus1(1,strideNumber-174:strideNumber-174+probStrides),...
    finalEstimateRotatedTranslatedhkplus1(2,strideNumber-174:strideNumber-174+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 1]);
plot(finalEstimateRotatedTranslatedhkplus2(1,strideNumber-173:strideNumber-173+probStrides),...
    finalEstimateRotatedTranslatedhkplus2(2,strideNumber-173:strideNumber-173+probStrides),'y-x','linewidth',1.5,'color',[0.5 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus3(1,strideNumber-172:strideNumber-172+probStrides),...
    finalEstimateRotatedTranslatedhkplus3(2,strideNumber-172:strideNumber-172+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 0]);
hold off; legend('hypothesis k-3 trajectory','hypothesis k-2 trajectory',...
    'hypothesis k-1 trajectory','hypothesis k trajectory',...
    'hypothesis k+1 trajectory','hypothesis k+2 trajectory',...
    'hypothesis k+3 trajectory');
...
probStridesVectorhkplus3 = zeros(1,probStrides);
for i=1:probStrides
    probStridesVectorhkmin3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin3(:,strideNumber-178+i),0,0,0,0);
    probStridesVectorhkmin2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin2(:,strideNumber-177+i),0,0,0,0);
    probStridesVectorhkmin1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin1(:,strideNumber-176+i),0,0,0,0);
    probStridesVectorhk(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhk(:,strideNumber-175+i),0,0,0,0);
    probStridesVectorhkplus1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus1(:,strideNumber-174+i),0,0,0,0);
    probStridesVectorhkplus2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus2(:,strideNumber-173+i),0,0,0,0);
    probStridesVectorhkplus3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus3(:,strideNumber-172+i),0,0,0,0);
end
...
% hkplus2angle = -36; % this is the most voted hypothesis
firstStride = 165; % 18
lastStride = strideNumber-175-1; % 16
roomStrides = lastStride-firstStride;
translationReference = finalEstimateRotatedTranslatedhk(:,lastStride);
for i = 0:roomStrides
    angleTemp = hkangle*(roomStrides-i)/roomStrides;
...
% plot(finalEstimateRotatedTranslatedhkplus2(1,19),finalEstimateRotatedTranslatedhkplus2(2,19),'ko','markersize',16);
hold off;
legend('complementary KF','non-recursive map-matching based MHT','start','stop');
set(legend,'fontsize',16,'interpreter','latex','location','northwest');
% figure(11);xlim([660 1410]);ylim([548 1018]);
% Room 221 traversal is over. The pedestrian is walking to PCV Lab now.
%% illustration of hypotheses - left off here, pick up from here
figure(22); set(figure(22),'position',[1 59 1114 671]);
set(gcf,'position',[564 61 927 670]);
imshow(img); hold on;
% 1upsamplingRate = 1; muUS = zeros(size(mu,1),upsamplingRate*size(mu,2));
% muUS(1,:) = interp(mu(1,:),upsamplingRate);
...
plot(finalEstimate(1,1),finalEstimate(2,1),'ro','linewidth',2,'markerfacecolor','r','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,end),finalEstimate(2,end),'gs','linewidth',2,'markerfacecolor','g','markeredgecolor','k','markersize',16);
plot(finalEstimate(1,strideNumber-94),finalEstimate(2,strideNumber-94),'kp','linewidth',2,'markerfacecolor','g','markersize',16);
plot(finalEstimate(1,strideNumber-93),finalEstimate(2,strideNumber-93),'kv','linewidth',2,'markerfacecolor','y','markersize',16);
plot(finalEstimate(1,strideNumber-92),finalEstimate(2,strideNumber-92),'ko','linewidth',2,'markerfacecolor',[1 0.5 0],'markersize',16);
plot(finalEstimate(1,strideNumber-91),finalEstimate(2,strideNumber-91),'ks','linewidth',2,'markerfacecolor',[1 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-90),finalEstimate(2,strideNumber-90),'kh','linewidth',2,'markerfacecolor',[0 0.5 1],'markersize',16);
plot(finalEstimate(1,strideNumber-89),finalEstimate(2,strideNumber-89),'kd','linewidth',2,'markerfacecolor',[0.5 0 0.5],'markersize',16);
plot(finalEstimate(1,strideNumber-88),finalEstimate(2,strideNumber-88),'k^','linewidth',2,'markerfacecolor',[0 0.5 0],'markersize',16);
% plot(finalEstimate(1,18),finalEstimate(2,18),'k<','linewidth',2,'markerfacecolor','k','markersize',16);
else (choice == 3)
plot(mu(1,1:end-30),mu(2,1:end-30), 'b-.','linewidth',2); % this is IMU (EKF+DR+ZUPT) solution
plot(finalEstimate(1,:),finalEstimate(2,:), 'r-','linewidth',2);
...
%% hypothesis k-3, k-2, k-1, k, k+1, k+2, k+3
% the angles are calculated according to IMU location & actual door location
hkmin3angle = -104; % rotation for hypothesis k-3, computed wrt k-3 and k-4
hkmin2angle = -40; % rotation for hypothesis k-2.
hkmin1angle = -40; % rotation for hypothesis k-1.
hkangle = -6*0.7-44*0.3; % -6 or -44 rotation for hypothesis k.
hkplus1angle = 40; % rotation for hypothesis k+1.
% burda kaldik kplus2
hkplus2angle = 60; % rotation for hypothesis k+2.
hkplus3angle = 45; % rotation for hypothesis k+3.
% for each hypothesis, the rotation matrix is wrt to actual door location
rotation_matrixkmin3angle = [cosd(hkmin3angle) -sind(hkmin3angle);
    sind(hkmin3angle) cosd(hkmin3angle)];
rotation_matrixkmin2angle = [cosd(hkmin2angle) -sind(hkmin2angle);
...
%door216Location = [760.4018;999.5193];
hkmin3Translation = [-1065;-1410]; % [-137;273]
finalEstimateRotatedTranslatedhkmin3 = finalEstimateRotatedhkmin3 + hkmin3Translation; % translation for hypothesis k-3
hkmin2Translation = finalEstimateRotatedhkmin2(:,strideNumber-93)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhkmin2 = finalEstimateRotatedhkmin2 - hkmin2Translation; % translation for hypothesis k-2
hkmin1Translation = finalEstimateRotatedhkmin1(:,strideNumber-92)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhkmin1 = finalEstimateRotatedhkmin1 - hkmin1Translation; % translation for hypothesis k-2
hkTranslation = finalEstimateRotatedhk(:,strideNumber-91)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhk = finalEstimateRotatedhk - hkTranslation; % translation for hypothesis k-2
hkplus1Translation = finalEstimateRotatedhkplus1(:,strideNumber-90)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhkplus1 = finalEstimateRotatedhkplus1 - hkplus1Translation; % translation for hypothesis k-2
hkplus2Translation = finalEstimateRotatedhkplus2(:,strideNumber-89)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhkplus2 = finalEstimateRotatedhkplus2 - hkplus2Translation; % translation for hypothesis k-2
hkplus3Translation = finalEstimateRotatedhkplus3(:,strideNumber-88)-finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94);%[-30; 140];
finalEstimateRotatedTranslatedhkplus3 = finalEstimateRotatedhkplus3 - hkplus3Translation; % translation for hypothesis k-2
probStrides = 10;
figure(23); set(figure(23),'position',[1 59 1114 671]);
set(gcf,'position',[564 61 927 670]);
imshow(img); hold on;
% 1upsamplingRate = 1; muUS = zeros(size(mu,1),upsamplingRate*size(mu,2));
% muUS(1,:) = interp(mu(1,:),upsamplingRate);
...
if (choice == 1 || choice == 2)
    if (showRotated)
        plot(finalEstimateRotatedhkmin1(1,1:end),finalEstimateRotatedhkmin1(2,1:end),'r-x','linewidth',1.5);
        plot(finalEstimateRotatedhkmin1(1,strideNumber-93),finalEstimateRotatedhkmin1(2,strideNumber-93),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhkmin1(1,strideNumber-92),finalEstimateRotatedhkmin1(2,strideNumber-92),'ks','linewidth',2,'markerfacecolor','y','markersize',16);
        plot(finalEstimateRotatedhkmin1(1,strideNumber-91),finalEstimateRotatedhkmin1(2,strideNumber-91),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
        % we can take stride k-2 to calculate the heading of the current hypothesis
        %plot(finalEstimateRotatedhkplus3(1,strideNumber-16),finalEstimateRotatedhkplus3(2,strideNumber-16),'ko','linewidth',2,'markerfacecolor','y','markersize',16);
    end
else (choice == 3)
...
plot(finalEstimateRotatedTranslatedhkmin3(1,strideNumber-94:strideNumber-94+probStrides),...
    finalEstimateRotatedTranslatedhkmin3(2,strideNumber-94:strideNumber-94+probStrides),'g-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin2(1,strideNumber-93:strideNumber-93+probStrides),...
    finalEstimateRotatedTranslatedhkmin2(2,strideNumber-93:strideNumber-93+probStrides),'y-x','linewidth',1.5);
plot(finalEstimateRotatedTranslatedhkmin1(1,strideNumber-92:strideNumber-92+probStrides),...
    finalEstimateRotatedTranslatedhkmin1(2,strideNumber-92:strideNumber-92+probStrides),'y-x','linewidth',1.5,'color',[1 0.5 0]);
plot(finalEstimateRotatedTranslatedhk(1,strideNumber-91:strideNumber-91+probStrides),...
    finalEstimateRotatedTranslatedhk(2,strideNumber-91:strideNumber-91+probStrides),'y-x','linewidth',1.5,'color',[1 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus1(1,strideNumber-90:strideNumber-90+probStrides),...
    finalEstimateRotatedTranslatedhkplus1(2,strideNumber-90:strideNumber-90+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 1]);
plot(finalEstimateRotatedTranslatedhkplus2(1,strideNumber-89:strideNumber-89+probStrides),...
    finalEstimateRotatedTranslatedhkplus2(2,strideNumber-89:strideNumber-89+probStrides),'y-x','linewidth',1.5,'color',[0.5 0 0.5]);
plot(finalEstimateRotatedTranslatedhkplus3(1,strideNumber-88:strideNumber-88+probStrides),...
    finalEstimateRotatedTranslatedhkplus3(2,strideNumber-88:strideNumber-88+probStrides),'y-x','linewidth',1.5,'color',[0 0.5 0]);
hold off; legend('hypothesis k-3 trajectory','hypothesis k-2 trajectory',...
    'hypothesis k-1 trajectory','hypothesis k trajectory',...
    'hypothesis k+1 trajectory','hypothesis k+2 trajectory',...
    'hypothesis k+3 trajectory');
...
probStridesVectorhkplus3 = zeros(1,probStrides);
for i=1:probStrides
    probStridesVectorhkmin3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin3(:,strideNumber-94+i),0,0,0,0);
    probStridesVectorhkmin2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin2(:,strideNumber-93+i),0,0,0,0);
    probStridesVectorhkmin1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkmin1(:,strideNumber-92+i),0,0,0,0);
    probStridesVectorhk(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhk(:,strideNumber-91+i),0,0,0,0);
    probStridesVectorhkplus1(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus1(:,strideNumber-90+i),0,0,0,0);
    probStridesVectorhkplus2(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus2(:,strideNumber-89+i),0,0,0,0);
    probStridesVectorhkplus3(i) = imcrop_taha_subpixel_accuracy(choice,finalEstimateRotatedTranslatedhkplus3(:,strideNumber-88+i),0,0,0,0);
end
...
%%
figure(24); set(gcf,'position',[213 187 1120 420]);
subplot(241);
stem(1:probStrides, probStridesVectorhkmin3,'g.','linewidth',1.7);
grid on; xlabel('stride');ylabel('probability');
title(['$\sum p_i \;=\;$', num2str(sum(probStridesVectorhkmin3)), ' for hypothesis k-3'],'interpreter','latex');
...
yticklabels({'','.2','.4','.6','.8','1'});
set(gca,'GridLineStyle','--','position',[0.1442 0.1221 0.2345 0.3216]);
%% gradual correction for the trajectory in PCV Lab.
% the hypothesis (stride) with the highest probability has the max rotation and 
% translation. the angle and the shift is gradually distributed to the strides in 
% the room. the very first stride in the room is applied least rotation and shift.
figure(25); set(figure(25),'position',[1 59 1114 671]);
set(gcf,'position',[564 61 927 670]);
imshow(img); hold on;
if (choice == 1 || choice == 2)
plot(mu(1,1:end),mu(2,1:end),'b-x','linewidth',1.5); % this is IMU (EKF+DR+ZUPT) solution
...
% hkplus2angle = -36; % this is the most voted hypothesis
firstStride = str5; % 18
lastStride = strideNumber-91-1; % 16
roomStrides = lastStride-firstStride;
translationReference = finalEstimateRotatedTranslatedhk(:,lastStride);
for i = 0:roomStrides
    angleTemp = hkangle*(roomStrides-i)/roomStrides;
...
% the very first stride in the room gets least rotation and shift
figure(26); set(figure(26),'position',[1 59 1114 671]);
set(gcf,'position',[564 61 927 670]);
imshow(img); hold on;
if (choice == 1 || choice == 2)
plot(mu(1,1:end),mu(2,1:end),'b-x','linewidth',1.5); % this is IMU (EKF+DR+ZUPT) solution
...
set(legend,'fontsize',16,'interpreter','latex','location','northwest');
% figure(11);xlim([660 1410]);ylim([548 1018]);
% Bolz Hall 2nd floor traversal is over.