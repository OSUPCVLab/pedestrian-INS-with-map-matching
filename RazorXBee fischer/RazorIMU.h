...
#define PACKET_FISCHER 26 // 12 bytes accelerations, 12 byte gyro readings, 2 byte time flag and packetNumber
...
struct PacketFischer {
	float accX, accY, accZ; // wrt body coordinate system
	float gyroX, gyroY, gyroZ; // wrt body coordinate system
	bool G_DtFlag; // integration time for gyros - see Razor_AHRS.ino and DCM.ino for more
	uint8_t packetNumber;
};
...
enum format { EAngles = 1, CSOutputs = 2, RCSOutputs = 3, PackageTaha = 4, PackageFischer = 5 };
